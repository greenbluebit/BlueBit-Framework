...
<?php 
	...
	$errorName = __( 'Please enter your name.', 'bluebit' );
	$errorEmail = __( 'Please enter a valid email address', 'bluebit' );
	$errorMessage = __( 'Please enter the message.', 'bluebit' );
	...
	// VALIDATION
		...
		// Check the name
		if( !bluebit_validate_length( $name, 3) ) {
			$isError	= true;
		...
		// Check the Message
		if( !bluebit_validate_length( $message, 2 ) ) {
			$isError	= true;
	...
	// SEND EMAIL IF NO ERROR
		...
		if( !isError ) {
			// get administrative email
			$emailReceiver = get_option( 'admin_email' );

			$emailSubject = sprintf( __( ' You have been contacted by %s', 'bluebit' ), $name );

			$emailBody = sprintf( __( ' You have been contacted by %s '. PHP_EOL .'With the following message: ', 'bluebit' ) . PHP_EOL . PHP_EOL, $name );

			$emailBody .= $message . PHP_EOL . PHP_EOL;

			$emailBody .= sprintf( __( 'You can contact %1$s via this email: %2$s', 'bluebit') . PHP_EOL . PHP_EOL, $name, $email );
...
<div class="main-content col-md-8" role='main'>
	...
	<article id="post-<?php the_ID();  ?>" <?php post_class() ?> > 
		...
		<header class="entry-header">
			<?php 
				// If post has thumbnail and not password protected, show thumbnail
		...
		<div class="entry-content">
			<?php if ( isset( $emailIsSent ) && $emailIsSent ) : ?>
				<div class="alert alert-success">
					<?php  _e( "Your message has been successfully send! You'll hear from us soon!", "bluebit" );?>
				</div> <!-- end of success alert -->

			<?php else : ?>

				<?php if( isset( $isError ) && $isError ) : ?>
					<div class="alert alert-error">
						<?php _e( "Oh dear! Something has not gone acording to plan. Could you tell the admin please?", "bluebit"); ?>
